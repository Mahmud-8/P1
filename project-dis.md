...
The Market Basket Analysis project applies association rule mining to discover hidden purchasing patterns in transactional retail data. Students will implement the **Apriori** and **FP-Growth** algorithms to uncover frequent itemsets and generate meaningful rules (e.g., *'customers who buy bread also buy butter'*). The insights will be presented through an interactive Streamlit or Dash dashboard that allows business teams to explore rules, filter by support and confidence thresholds, and generate product recommendations in real time.
...
1. **Data Collection:**
   - Acquire transactional datasets from sources such as Kaggle (Online Retail II, Groceries Dataset, Instacart) or UCI Repository.
...
9. **Algorithm Implementation:**
   - Implement Apriori using the `mlxtend` library. Experiment with different minimum support thresholds (e.g., 0.01, 0.02, 0.05).
...
14. **Streamlit / Dash App Features (All MUST be included):**
    - **Rule Explorer:** Searchable, filterable table of all mined rules.
    - **Product Recommender:** User selects items in a basket and gets top recommendations.
    - **Network Graph Tab:** Shows product association clusters interactively (use Plotly or Pyvis).
    - **Threshold Sliders:** For support, confidence, and lift to dynamically update displayed rules.
